...
const MovieDetails = () => {
  ...
  const getShow = async () => {
    try {
      ...
      if (data.success && data.movie) {
        ...
        const normalisedMovie = {
          ...apiMovie,
          overview: apiMovie.Overview ?? apiMovie.overview,
          poster_path: apiMovie.Poster_path ?? apiMovie.poster_path,
          genres: apiMovie.geners ?? apiMovie.genres,
        };

        setShow({
          movie: normalisedMovie,
  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
      <div className='flex flex-cols md:flex-row gap-8 max-w-6xl mx-auto'>
        <img src={image_base_url + show.movie.poster_path} className=' max-md:mx-auto rounded-xl
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <p className=''>{timeFormat(show.movie.runtime)} . {show.movie.genres.map(genre =>
            genre.name).join(", ")} .{show.movie.release_date.split("-")[0]} </p>

          <div className='flex items-center flex-wrap gap-4 mt-4'>
            <button className='flex items-center gap-2 px-7 py-3 text-sm
              bg-green-800 hover:bg-gray-900 transition rounded-md font-medium
              cursor-pointer active:scale-95'>
              <PlayCircleIcon className="w-5 h-5" />
              Watch Trailer</button>
            <a href='#dateSelect' className='px-10 py-3 taxt-sm bg-primary hover:bg-primary-dull
